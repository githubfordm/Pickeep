...
//        });
    ...
    private void setList() {
        MediaStoreHelper.getList(this, new FileResultCallback<Document>() {
            @Override
            public void onResultCallback(List<Document> files) {
                setDataOnShelf(files);
            }
        });
    }
    */
    ...
    private void setList()
    {
        ...
        {
            String title = getTitleFromPath(list[i].getPath()); Log.i("zxc","title = "+title);
            Bitmap src = BitmapFactory.decodeFile(list[i].getPath() +"/"+ title + ".jpg"); Log.i("zxc","image = "+list[i].getPath() + title + ".jpg");
            bookArray[i+1] = src;
    ...
    private void setDataOnShelf(List<Document> documents)
    {
        int size = documents.size();
        Log.i("zxc","size() = "+size);
        for(int i=0; i<size; i++)
        {
            File folder = new File(documents.get(i).getPath()); Log.i("zxc","folder path = "+folder.getPath());
            String title = getTitleFromPath(documents.get(i).getPath()); Log.i("zxc","title = "+title);
            Bitmap src = BitmapFactory.decodeFile(folder.getPath() + title + ".jpg"); Log.i("zxc","image = "+folder.getPath() + title + ".jpg");
            bookArray[i+1] = src;
            scanning(i+1,folder.getPath());
        }
    }
